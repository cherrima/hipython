...
try:
    ...
    from predictor import predict_peak
except ImportError:
...
if menu == "대시보드":
    ...
    with top_col1:
        ...
        # 1. 날씨 정보를 담을 빈 컨테이너 생성
        weather_area = st.empty()
        
        # 2. 날씨 정보를 그리는 헬퍼 함수 (반복 사용을 위함)
        def display_weather():
            w = get_weather_info()
            with weather_area.container():
                c1, c2, c3, c4, c5 = st.columns(5)
                c1.metric("상태", f"☀️ {w['status']}")
                c2.metric("온도", f"{w['temperature']}°C")
                c3.metric("습도", f"{int(w['humidity'])}%")
                c4.metric("풍속", f"{w['windspeed']}m/s")
                c5.metric("강수", f"{w['rainfall']}mm")
                st.caption(f"🏠 울산 울주군 삼동면 암리 | {w['date']} {w['hour']}:00 기준")

        # 3. 버튼 클릭 시 rerun 없이 날씨 함수만 재실행
        if w_h2.button("🔄 날씨 갱신"):
            display_weather()
            st.toast("날씨 정보가 업데이트되었습니다!") # 사용자 알림용 (선택)
        else:
            # 초기 로드 시 실행
            display_weather()
    ...
    with top_col2:
        ...
        with st.container(border=True):
            ...
            if e2.button("수정 적용"):
                ...
                st.rerun() # 설정값 변경 시엔 전체 반영을 위해 유지하는 것이 좋습니다.
    ...
    print("Target Date:", target_dtstr)
    df_res_raw = get_daily_result(target_dtstr)
    df_pre_raw = predict_peak(get_prediction_variables(target_dtstr))
    
    # 'date' 컬럼 추가
    df_pre_raw['date'] = target_date
